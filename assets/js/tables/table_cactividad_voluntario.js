...
var handleDataTableResponsive = function() {
	...
	if ($('#tbl_actividades_voluntarios').length !== 0) {
		table = $('#tbl_actividades_voluntarios').DataTable({
			...
			ajax: {
				"method": "POST",
				"dataType": "json",
				"url": "./includes/fnc_datatbl.php?caso=datatbl_actividades_voluntarios"
			},
			...
			columnDefs: [
				{ targets: 6, visible: true },
				{
					...
					render: function (data, type, row, meta) {
						let fila = meta.row;
						let id = data["id_tarea_voluntario_pk"];
						let botones = '<button type="button" class="btn btn-outline-warning btn-xs" onclick="modificar_actividad_voluntario(' + id + '); return false;"><i class="fa fa-edit" style="font-size:10px;"></i></button><button type="button"  class="btn btn-outline-danger btn-xs" onclick="eliminar_actividad_voluntario(' + id + '); return false;"><i class="fa fa-trash" style="font-size:10px;"></i></button>';	
						return botones;
					}
				}
			],
