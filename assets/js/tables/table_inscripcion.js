...
var handleDataTableResponsive = function() {
	...
	if ($('#tbl_inscripcion').length !== 0) {
		table = $('#tbl_inscripcion').DataTable({
			"destroy": true,
			"fnDestroy": true,
			"ajax": {
				"method": "POST",
				"dataType": "json",
				"url": "./includes/fnc_datatbl.php?caso=datatbl_tbl_inscripcion"
			},
			...
			columnDefs: [
				{ targets: 3, visible: true },
				{
					...
					render: function (data, type, row, meta) {
						let fila = meta.row;
						let id = data["id_costo_pk"];
						let botones = '<button type="button" class="btn btn-outline-warning btn-xs" onclick="actualiza_inscripcion_kit('+id+'); return false;"><i class="fa fa-edit" style="font-size:10px;"></i></button><button type="button"  class="btn btn-outline-danger btn-xs" onclick="elimina_inscripcion_kit('+id+'); return false;"><i class="fa fa-trash" style="font-size:10px;"></i></button>';	
						return botones;
					}
				}
			],
