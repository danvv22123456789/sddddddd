...
var handleDataTableResponsive = function() {
	...
	if ($('#tbl_trabajos_ep').length !== 0) {
		$('#tbl_trabajos_ep').DataTable({
			...
			ajax: {
				"method": "POST",
				"dataType": "json",
				"url": "./includes/fnc_datatbl.php?caso=datatbl_trabajos_ep"
			},
			...
			columnDefs: [
				{ targets: 7, visible: false },
				{
					...
					render: function (data, type, row, meta) {
						let fila = meta.row;
						let botones = `
								<button type="button" class='btn btn-outline-primary btn-xs' title='Asignar revisores' onclick='asignarrevisores( ".$resvi_traba." );'><i class="fa fa-users" style="font-size:10px;"></i></button>
								<button type="button"  class='btn btn-outline-danger btn-xs' title='cancelar solicitud' onclick='cancelarsolicitud(".$resvi_traba.");' ><i class="fa fa-times" style="font-size:10px;"></i></button>`;	
						return botones;
					}
				}
			],
