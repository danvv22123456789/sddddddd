...
var handleDataTableResponsive = function() {
	...
	if ($('#tbl_premio_a_trabajo').length !== 0) {
		$('#tbl_premio_a_trabajo').DataTable({
			...
			ajax: {
				"method": "POST",
				"dataType": "json",
				"url": "./includes/fnc_datatbl.php?caso=datatbl_premio_a_trabajo"
			},
			...
			columnDefs: [
				{ targets:5, visible: false },
				{
					...
					render: function (data, type, row, meta) {
						let fila = meta.row;
						let botones = `
								<button type="button" class='btn btn-outline-warning btn-xs' onclick='toggle("#div_misAcciones"); return false;'><i class="fa fa-list" style="font-size:10px;"></i></button>
								<button type="button"  class='btn btn-outline-success btn-xs' onclick='toggle("#div_misAcciones"); return false;'><i class="fa fa-refresh" style="font-size:10px;"></i></button>`;	
						return botones;
					}
				}
			],
