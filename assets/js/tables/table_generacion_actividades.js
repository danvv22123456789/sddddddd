...
var handleDataTableResponsive = function() {
	...
	if ($('#tbl_generacion_actividades').length !== 0) {
		table = $('#tbl_generacion_actividades').DataTable({
			"destroy": true,
			"fnDestroy": true,
			ajax: {
				"method": "POST",
				"dataType": "json",
				"url": "./includes/fnc_datatbl.php?caso=datatbl_tbl_actividades"
			...
			columnDefs: [
				{ targets: 5, visible: true },
				{
					...
					render: function (data, type, row, meta) {
						let fila = meta.row;
						let id = data["id_actividad_pk"];
						let botones = '<button type="button" class="btn btn-outline-warning btn-xs" onclick="modificar_actividad(' + id + '); return false;"><i class="fa fa-edit" style="font-size:10px;"></i></button><button type="button"  class="btn btn-outline-danger btn-xs" onclick="eliminar_actividad(' + id + '); return false;"><i class="fa fa-trash" style="font-size:10px;"></i></button>';
