...
var handleDataTableResponsive = function() {
	...
	if ($('#tbl_tematica').length !== 0) {

		table = $('#tbl_tematica').DataTable({
			"destroy": true,
			"fnDestroy": true,
			"ajax": {
				"method": "POST",
				...
				columnDefs: [
					{ targets: 4, visible: true },
					{
						...
						render: function (data, type, row, meta) {
							let fila = meta.row;
							let id = data["id_tematica_pk"];
							let botones = '<button type="button" class="btn btn-outline-warning btn-xs" onclick="actualiza_tematica('+id+'); return false;"><i class="fa fa-edit" style="font-size:10px;"></i></button><button type="button"  class="btn btn-outline-danger btn-xs" onclick="elimina_tematica('+id+'); return false;"><i class="fa fa-trash" style="font-size:10px;"></i></button>';	
							return botones;
						}
					}
				],
