...
var handleDataTableResponsive = function() {
	...
	if ($('#tbl_certificado').length !== 0) {
		$('#tbl_certificado').DataTable({
			"destroy": true,
			"fnDestroy": true,
			ajax: {
				"method": "POST",
				"dataType": "json",
				"url": "./includes/fnc_datatbl.php?caso=datatbl_certificados"
			...
			columnDefs: [
				{ targets: 3, visible: true },
				{
					...
					render: function (data, type, row, meta) {
						let fila = meta.row;
						let id = data["id_certificado_pk"];
						let botones = '<button type="button" class="btn btn-outline-warning btn-xs" onclick="modificar_certificado(' + id + '); return false;"><i class="fa fa-edit" style="font-size:10px;"></i></button><button type="button"  class="btn btn-outline-danger btn-xs" onclick="eliminar_certificado(' + id + '); return false;"><i class="fa fa-trash" style="font-size:10px;"></i></button>';	
						return botones;
					}
				}
			],
