...
class solicitud {
    ...
    public function screar()
    {
       $pdo = $this->base->abrir_conexion();    
        $datos=$pdo->prepare("INSERT INTO tbl_solicitud(id_solicitud, id_tipo_solicitud, motivo_solicitud, status, fecha_solicitud, fecha_resolucion, id_usuario_pk)"
                                                         . " VALUES (NULL,?,?,?,?,NULL,?),'isssi'");
        $resultado=$datos->execute([$this->tsolicitud,$this->motivo,$this->status,$this->fsolicitud,$this->idusuario], TRUE );
//        echo "INSERT INTO tbl_solicitud(id_solicitud, id_tipo_solicitud, motivo_solicitud, status, fecha_solicitud, fecha_resolucion) VALUES (NULL,$this->tsolicitud,$this->motivo,$this->status,$this->fsolicitud,NULL)";
    ...
    {
        $pdo = $this->base->abrir_conexion();  
        $datos = $pdo ->prepare("UPDATE tbl_solicitud SET status = '1' , fecha_resolucion=? WHERE id_solicitud = ?,'si'");
        $resultado=$datos->execute([$this->fresolucion ,$this->idsolicitud]);
    ...
    {
        $pdo = $this->base->abrir_conexion();  
        $datos = $pdo ->prepare("UPDATE tbl_solicitud SET status = '2' , fecha_resolucion=? WHERE id_solicitud = ?,'si'");
        $resultado=$datos->execute([$this->fresolucion ,$this->idsolicitud]);
    ...
    {
        $pdo = $this->base->abrir_conexion();
        $datos = $pdo ->prepare("select nombre_tipo_solicitud from tbl_tipo_solicitud where id_tipo_solicitud =".$this-> tsolicitud."");
        return $datos;
    }    
    ...
    {
        $pdo = $this->base->abrir_conexion();
         $verificador = $pdo->prepare("INSERT INTO tbl_solicitud_roles_congreso(id_solicitud, id_rol_congreso_pk) "
                 . "VALUES (?,?),'ii'");
                 $resultado=$verificador->execute ([$this->idsolicitud,$this->congreso_rol],TRUE);
//                 echo "INSERT INTO tbl_solicitud_usuario_congreso_roles(id_solicitud, id_usuario_congreso_rol_pk) VALUES ($this->idsolicitud,$this->congreso_rol)";
         ...
         if($verificador!=0)
         {
            $pdo->prepare("DELETE FROM tbl_solicitud WHERE id_solicitud = ?,'i'",[$this->idsolicitud]);
            return -1;
         }else{
                return 1;
         }                      
    ...
    public function sol_tematica()
    {
        ...
        for ($i=0;$i<count($this->tematica);$i++)
        {
            ...
            $verificador=$pdo->prepare("INSERT INTO tbl_solicitud_tematica(id_solicitud, id_tematica_pk)"
                    . " VALUES (?,?),'ii'");
                    $resultado=$verificador->execute([$this->idsolicitud,$tematica] );
            if(($verificador != -1) || ($verificador != 0))
            {
                $verificador = $i;
            }else{
                $verificador=$pdo->prepare("delete from tbl_solicitud_tematica where id_solicitud =? and id_tematica_pk = ?"
                    . " ". "ii");
                    $resultado=$verificador->execute([$this->idsolicitud,$tematica], TRUE );     
                return -1;
            }
        }
//        echo $verificador.count($this->tematica);
    ...
    {
        $pdo = $this->base->abrir_conexion();
        $datos = $pdo->prepare("INSERT INTO tbl_solicitud_certificados(id_solicitud, id_certificado_pk)"
                    . " VALUES (?,?),'ii'");
                    $resultado=$datos->execute([$this->idsolicitud, $this->certificado], TRUE );
    ...
    {
        $pdo = $this->base->abrir_conexion();
        $datos= $pdo->prepare("INSERT INTO tbl_solicitud_idioma(id_solicitud, id_idioma_pk)"
                    . " VALUES (?,?),'is'");
                    $resultado=$datos->execute([$this->idsolicitud, $this->idioma], TRUE );
    }      
