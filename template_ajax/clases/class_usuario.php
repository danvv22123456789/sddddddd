...
class usuario
{
        ...
        */
        ...
        if (!empty($contrasena)) {
            foreach ($contrasena as $contra) {
                $encriptado_db = $contra['contrasena'];
                if (password_verify($this->contrasenia, $encriptado_db)) {
                    return 1;
                } else {
                    return 0;
                }
    ...
    public function asignar_usuarioxrolxcongreso()
    {
        ...
        $pdo = $this->base->abrir_conexion();
        $datos =  $pdo->prepare("INSERT INTO tbl_usuario_congreso_roles(tbl_usuario_congreso_rol_pk,id_usuario_fk,
            id_rol_congreso_fk, asistira)
           VALUES (?,?,?,?)");

        $resultado = $datos->execute([NULL, $this->idusuario, $this->rol, 1]);
    ...
    {
        $pdo = $this->base->abrir_conexion();
        $datos   = $pdo->prepare("INSERT INTO tbl_congreso_rol_tematicas(id_tematica_fk, id_usuario_congreso_roles_fk) "
            . "VALUES (?,?)");
        $resultado = $datos->execute([$this->tematica, $this->usuario_congreso_rol], TRUE);
        echo $resultado;
    }
    ...
    {
        $pdo = $this->base->abrir_conexion();
        $datos =  $pdo->prepare("UPDATE tbl_usuario SET img_usuario =? WHERE id_usuario_pk = ?");

        $resultado = $datos->execute([
            $this->img,  $this->idusuario
        ]);
    ...
    public function envio_contra()
    {
        ...
        $pdo = $this->base->abrir_conexion();
        $resp =  $pdo->prepare("UPDATE tbl_usuario SET contrasena = '" . $ncontrasenia . "' WHERE id_usuario_pk = " . $this->idusuario . "");
    ...
    public function update_nusuario()
    {
        ...
        $datos =  $pdo->prepare("UPDATE tbl_usuario SET nombre_usuario = ?, contrasena=? WHERE id_usuario_pk = ?");

        $resultado = $datos->execute([
            $this->nusuario, $this->contrasenia, $this->idusuario
        ]);
    }
...
public function tabs_gestion_roles($id_rol)
{

    $pdo = $this->base->abrir_conexion();
    // $datos = $pdo->query("select distinct a.id_usuario_pk, a.nombre_usuario, CONCAT_WS(' ',b.primer_nombre, b.segundo_nombre,b.primer_apellido,b.segundo_apellido) as nombres from tbl_usuario a
    // join tbl_persona b on b.id_persona_pk=a.id_persona_fk
    // join tbl_usuario_congreso_roles c on c.id_usuario_fk=a.id_usuario_pk
    // join tbl_roles_congreso d on d.tbl_rol_congreso_pk=c.id_rol_congreso_fk  
    // join tbl_roles e on e.id_rol_pk=d.id_rol_fk 
    // join tbl_solicitud f on f.id_usuario_pk!=a.id_usuario_pk where e.id_rol_pk='".$id_rol."'");

    $datos = $pdo->query("SELECT distinct a.id_usuario_pk, a.nombre_usuario,CONCAT_WS(' ',b.primer_nombre, b.segundo_nombre,b.primer_apellido,b.segundo_apellido) as nombres,c.id_rol_congreso_fk FROM tbl_usuario a  join tbl_persona b on b.id_persona_pk=a.id_persona_fk join tbl_usuario_congreso_roles c on c.id_usuario_fk=a.id_usuario_pk join tbl_roles e on e.id_rol_pk=c.id_rol_congreso_fk where c.id_rol_congreso_fk='".$id_rol."'");
    return $datos;
}
    ...
    public function interes()
    {
        ...
        $datos = $pdo->query("select * from tbl_solicitud a
            ...
            where a.id_tipo_solicitud='1' and a.status='0' and g.id_congreso_pk=1 and a.id_solicitud=1");
        return $datos;
    }
}
...
class editorprincipal extends usuario
{
  ...
  */
      ...
      (?,?,?,?,?,?,?,?,?)");
        $resultado = $datos->execute([
            $this->idtabla,  $this->pendiente,  $this->aceptado, $this->fecha_acepto_rechazo,
    ...
    public function cancelar_revisor_trabajo($id)
    {

        $pdo = $this->base->abrir_conexion();
        $dato = $pdo->prepare("DELETE from tbl_asignacion_a_revisor where id_trabajo_fk=? and id_usuario_que_recibe=?");
        try {
            $dato->execute([$id]);
    ...
    public function dictaminar_trabajo()
    {
        ...
        $pdo = $this->base->abrir_conexion();
        $datos =  $pdo->prepare("UPDATE tbl_asignacion_a_revisor set id_tipo_dictamen_fk=? where id_trabajo_fk=? ");

        $resultado = $datos->execute([
            $this->dictamen, $this->idtrabajo
        ]);
    }
...
class editor_secundario_seccion extends usuario
{
    ...
    public function  asignar_editor_secundario_seccion()
    {
        ...
        $pdo = $this->base->abrir_conexion();
        $datos   = $pdo->prepare("INSERT into tbl_asignacion_editor_seccion_secundario(id_asignacion_editor_seccion_secundaria_pk, fecha_recibe_asignacion, id_usuario_que_asigna, id_usuario_que_recibe, id_trabajo_fk) values (?,?,?,?,?)");
        $resultado = $datos->execute([$this->id_asignacion_editor_seccion_secundaria_pk, $this->fecha_recibe_asignacion, $this->id_usuario_que_asigna, $this->id_usuario_que_recibe, $this->id_trabajo_fk], TRUE);
        echo $resultado;
    }
}
...
class revisor extends usuario
{
    ...
    public function irespuesta_cualitativa()
    {
        ...
        $pdo = $this->base->abrir_conexion();
        $datos   = $pdo->prepare("INSERT into tbl_respuesta_cualitativa(id_respuesta_cualitativa_pk,respuesta_cualitativa, id_pregunta_cualitativa_fk) values (?,?,?)");
        $resultado = $datos->execute([$this->id_respuesta_cualitativa_pk,  $this->respuesta_cualitativa, $this->id_pregunta_cualitativa_fk], TRUE);
        echo $resultado;
    }
    public function irespuesta_cuantitativa()
    {
        ...
        $pdo = $this->base->abrir_conexion();
        $datos   = $pdo->prepare("INSERT into tbl_respuesta_cuantitativa(id_respuesta_cuantitativa_pk,respuesta_cuantitativa, 
        id_pregunta_cuantitativa_fk) values (?,?,?)");
        $resultado = $datos->execute([$this->id_respuesta_cuantitativa_pk,  $this->respuesta_cuantitativa, $this->id_pregunta_cuantitativa_fk], TRUE);
        echo $resultado;
    }
    public function irevisiontrabajo()
    {
        ...
        $pdo = $this->base->abrir_conexion();
        $datos   = $pdo->prepare("INSERT into tbl_revisiones_trabajo(id_revisiones_trabajo_pk, id_asignacion_a_revisor_fk, 
        descargo_archivo, lleno_formulario, fecha_reviso, id_tipo_dictamen_fk, observaciones) values (?,?,?,?,?,?,?)");
        $resultado = $datos->execute([
            $this->id_revisiones_trabajo_pk, $this->id_asignacion_a_revisor_fk, $this->descargo_archivo, $this->lleno_formulario,
            $this->fecha_reviso, $this->id_tipo_dictamen_fk, $this->observaciones
        ], TRUE);
        echo $resultado;
    }
    public function irespuestas_revisiones_trabajos_cuantitativas()
    {
        ...
        $pdo = $this->base->abrir_conexion();
        $datos   = $pdo->prepare("INSERT into tbl_respuestas_revisiones_trabajos_cuantitativas(id_revisiones_trabajo_fk, 
        id_respuesta_cuantitativa_fk) values (?,?)");
        $resultado = $datos->execute([$this->id_revisiones_trabajo_pk,  $this->id_respuesta_cuantitativa_pk], TRUE);
        echo $resultado;
    }
    public function irespuestas_revisiones_trabajos_cualitativas()
    {
        ...
        $pdo = $this->base->abrir_conexion();
        $datos   = $pdo->prepare("INSERT into tbl_respuestas_revisiones_trabajos_cualitativas(id_revisiones_trabajo_fk, 
        id_respuesta_cualitativa_fk) values (?,?)");
        $resultado = $datos->execute([$this->id_revisiones_trabajo_pk,  $this->id_respuesta_cualitativa_pk], TRUE);
        echo $resultado;
    }
    ...
    public function aceptar_cancelar_trabajo($pendiente, $aceptado, $fecha_acepto_rechazo, $idtrabajo, $idtabla)
    {

        $pdo = $this->base->abrir_conexion();
        $datos =  $pdo->prepare("update tbl_asignacion_a_revisor set pendiente_aceptacion=? , aceptado=?, fecha_acepto_rechazo=? where id_trabajo_fk=? and id_asignacion_a_revisor_pk=?");
        $resultados = $datos->execute([$pendiente, $aceptado, $fecha_acepto_rechazo, $idtrabajo, $idtabla]);
        return $resultados;
    }


    public function cancelar_trabajo($id)
    {

        $pdo = $this->base->abrir_conexion();
        $dato = $pdo->prepare("DELETE from tbl_asignacion_a_revisor where id_trabajo_fk=? and id_asignacion_a_revisor_pk=?");
        try {
            $dato->execute([$id]);
        } catch (PDOException $e) {
            echo "Error al ejecutar la consulta: " . $e->getMessage();
...
class voluntario extends usuario
{
    ...
    public function inicia_aceptar_rechazar_voluntario($dictamen, $id_solicitud)
    {
        ...
        $pdo = $this->base->abrir_conexion();
        $datos =  $pdo->prepare("UPDATE tbl_solicitud set status='" . $dictamen . "' where id_solicitud = " . $id_solicitud . "");
    }
    ...
    *Creacion de SELECT tbl_usuario_congreso_rol_pk
    ...
    public  function crea_voluntario()
    {
      ...
      id_rol_congreso_fk) VALUES(?,?,?,?,?)");
        $resultado = $datos->execute([
            $this->id_voluntario_pk, $this->numero_horas, $this->comentarios, $this->estado,
...
*/
    ...
    */
    ...
    function tbl_tarea_voluntario()
    {
        $pdo = $this->base->abrir_conexion();

        $datos = $pdo->query("SELECT * from tbl_tarea_voluntario");
        return $datos;
    }
...
*/
    ...
    */
    ...
    {
        $pdo = $this->base->abrir_conexion();
        $datos =  $pdo->prepare("UPDATE tbl_voluntario set estado=1 where id_voluntario_pk = " . $idvoluntario . "");
    }
