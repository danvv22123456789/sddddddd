...
public function crear_congreso_roles($roles, $idcongreso) {
    $pdo = $this->base->abrir_conexion();
    $resultado = true;
    foreach ($roles as $id_rol) {
        $datos = $pdo->prepare("INSERT INTO tbl_roles_congreso (id_rol_fk, id_congreso_fk) VALUES (?, ?)");
        $resultado = $resultado && $datos->execute([$id_rol, $idcongreso]);
    }
    return $resultado;
}

public function getCongreso($id) {
    $bdd = new basedatos();
    $datos = $bdd ->select("select a.id_congreso_pk,a.id_pais_fk,a.logo_congreso, a.nombre_congreso,a.numero_cai,a.anio, a.descripcion_congreso, a.siglas,a.coordenadas,a.lema,a.fecha_f_recepcion,a.fecha_i_recepcion,a.fecha_i_revision,a.fecha_f_revision,a.fecha_p_programa,a.fecha_cambio_costo_inscripcion, b.nombre_estado, c.nombre_pais,a.lugar, a.fecha_inicio, a.fecha_finalizacion from tbl_congreso a 
    ...
    where 1=1 and a.id_estado_congreso_fk > 0 and a.id_congreso_pk='$id'");
    return $datos;   
}
...
public function mostrarRoles($idcongreso) {
    $bdd = new basedatos();
    $datos = $bdd ->select("SELECT a.id_congreso_fk, a.id_rol_fk, b.nombre_rol  
...
public function select_roles_congreso(){
        $pdo = $this->base->abrir_conexion();    
    ...
    public  function slc_pais(){
        $pdo = $this->base->abrir_conexion();    
        $datos = $pdo->query("select * from tbl_pais where 1=1");
        return $datos;
 ...
 */
   ...
   $this->lugar, $this->coordenadas, $this->id_pais_fk, $this->logo_congreso, $this->lema, $this->numero_cai, 
...
*/
      $datos =  $pdo->prepare("INSERT INTO tbl_log(id_log_pk,id_usuario_fk, fecha, hora, ip,informacion_extra,  id_tipo_log_fk) VALUES (?,?,?,?,?,?,?)");
    $ip = $_SERVER['REMOTE_ADDR'] ? $_SERVER['REMOTE_ADDR'] : 'null';
      $resultado=$datos->execute([NULL,$this->idusuario_cm, date("y-m-d"), date('H:i:s'),
      $ip, "Modificación de información básica del Congreso" ,2]);
        
}

    public function borrar_roles_congreso($idcongreso)
    {
        $pdo = $this->base->abrir_conexion();  
        $linea_borrada      =      $pdo->prepare("DELETE FROM `tbl_roles_congreso` WHERE `id_congreso_fk`=?");
        $linea_borrada      ->      execute([$this->id_congreso_pk]);
    }
...
public function inactivar() {
    if(  self::congreso_activo()  ) {
       ...
       */
       ...
       /*
       UPDATE e INSERT CON METODO PDO
       */
      $pdo = $this->base->abrir_conexion();    
      $datos =  $pdo->prepare("UPDATE tbl_congreso SET id_estado_congreso_fk=?,
      fecha_modificacion=?, modificado_por=?  WHERE id_congreso_pk=?");
...
*/
    ...
    /*
    Creacion de  DELETE  en la tabla tbl_roles_congreso
    con metodo PDO
*/
        $linea_borrada      =      $pdo->prepare("DELETE FROM `tbl_roles_congreso` WHERE `id_congreso_fk`=?");
        $linea_borrada      ->      execute([$this->id_congreso_pk]);
    ...
    /*
    Creacion de  DELETE  en la tabla tbl_congreso con metodo PDO
*/
...
public function json_usuarios_congreso() {
   // require_once '../funciones/funcion_traducir.php';
    ...
    $pdo = $this->base->abrir_conexion();  
    $usuarios            =   $pdo->query("select DISTINCT(id_usuario_pk), nombre_usuario, correo from tbl_usuario a, tbl_persona b, tbl_correo c, tbl_usuario_congreso_roles d, tbl_roles_congreso e
    ...
    $roles_por_usuario   =   $pdo->query("select id_usuario_pk, id_rol_fk from tbl_usuario a, tbl_persona b, tbl_correo c, tbl_usuario_congreso_roles d, tbl_roles_congreso e
                                                where a.id_persona_fk = b.id_persona_pk and b.id_persona_pk = c.id_persona_fk and a.id_usuario_pk = d.id_usuario_fk and d.id_rol_congreso_fk = e.tbl_rol_congreso_pk
                                                and e.id_congreso_fk = " . $this->id_congreso_pk . " and a.id_usuario_pk !=".$this->idusuario_cm."");
    $arreglo             =    array();
...
private function eliminar_logo_congreso($logo) {
    ...
    public function get_congreso($idcongreso, $nusuario, $idusuario)
{
    $bdd = new basedatos();
    $datos = $bdd ->select("select * from tbl_usuario a, tbl_persona b, tbl_tipo_persona c, tbl_idioma d, tbl_tipo_alimentacion e, tbl_tipo_identificacion f, tbl_pais g, tbl_correo h, tbl_telefono i
                where a.nombre_usuario= ". $nusuario ."
                and a.id_usuario_pk=". $idusuario. "
...
public function slc_congresos(){
    $pdo = $this->base->abrir_conexion();    
    $datos = $pdo->query("SELECT * FROM tbl_congreso WHERE 1=1");
    return $datos;
