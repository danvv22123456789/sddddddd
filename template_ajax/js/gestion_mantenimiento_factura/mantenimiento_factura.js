...
$('#crear_tour').click(function (){
     $.post("./form/congresos/form_tour.php", {}, function (resp) {     
       console.log(resp);
       $('#contenedor').html("");
       retorno(resp);
        b_func('tbl_tours');
...
$('#btn_crear_tour').click(function(){
    ...
    if (vacio == 0){
        ...
        if (caso == "guardar_tour"){
            ...
            var ruta = "./form/congresos/plantillas/funciones.php?caso="+caso;
            $.ajax({
                url: ruta,
                type: "POST",
                data: formData,
            ...
            $.post("./form/congresos/plantillas/funciones.php?caso="+caso,{idtour:id,nombretour:nombretour,descripcion:descripcion,comentario:comentario,costo:costo},function(resp){
             //alert(resp)
                if(resp){
...
function llenar_tour(id){
    $.post("./form/congresos/plantillas/funciones.php?caso=llenar_form",{idtour:id},function(resp){llenar_tours(resp);});
...
$('#inscripcion_kit').click(function (){
    $.post("./form/congresos/form_inscripcion_kit.php", {}, function (resp) {     
       //console.log(resp);
       $('#contenedor').html("");
       retorno(resp);
...
$("#btn_guardar_articulo").click(function(){
    ...
    if (error1 == 1){
        ...
        if (caso == 'guardar_articulo'){
            ...
            $.post("./form/congresos/plantillas/funciones.php?caso="+caso,{id:id,nombre:nombre,descripcion:descripcion,precio:precio,impuesto:impuesto},function(resp){
                //console.log(resp);
                if(resp){
...
function llenar_articulo_costo(id){
   $.post("./form/congresos/plantillas/funciones.php?caso=llenar_articulos",{idarticulo:id},function(resp){llenar_articulo(resp);});
}
